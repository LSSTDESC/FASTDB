# This has a lot of redundant code wtih load_snana_fits.py, but the
# regular and ppdb tables have diverged a fair bit by now.
...
from db import DB, PPDBDiaObject, PPDBHostGalaxy, PPDBDiaSource, PPDBDiaForcedSource
    ...
    def __call__( self ):
        ...
        try:
            ...
            with DB() as conn:
                ...
                cursor.execute( "UPDATE ppdb_diaobject o SET nearbyextobj1id=h.id "
                                "  FROM ppdb_host_galaxy h "
                                "  WHERE o.nearbyextobj1=h.objectid" )
                FDBLogger.info( "...nearbyextobj2..." )
                cursor.execute( "UPDATE ppdb_diaobject o SET nearbyextobj2id=h.id "
                                "  FROM ppdb_host_galaxy h "
                                "  WHERE o.nearbyextobj2=h.objectid" )
                FDBLogger.info( "...nearbyextobj2..." )
                cursor.execute( "UPDATE ppdb_diaobject o SET nearbyextobj3id=h.id "
                                "  FROM ppdb_host_galaxy h "
