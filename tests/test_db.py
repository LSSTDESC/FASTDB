...
def test_construct_sql_where_clause():
    ...
    q, subdict, missing = construct_pgsql_where_clause( searchspec, just="A" )
    assert missing == set()
    assert q.as_string() == 'WHERE "just"=%(just)s'
    assert subdict == { 'just': 'A' }

    q, subdict, missing = construct_pgsql_where_clause( searchspec, just="A", mult="B", does_not_exist=42 )
    assert missing == set( [ 'does_not_exist' ] )
    assert q.as_string() == 'WHERE "just"=%(just)s AND "mult"=%(mult)s'
    assert subdict == { 'just': 'A', 'mult': 'B' }

    q, subdict, missing = construct_pgsql_where_clause( searchspec, just="A", mult=[ "B", "C" ] )
    assert missing == set()
    ...
    q, subdict, missing = construct_pgsql_where_clause( searchspec, substr="B", substr_contains="C" )
    assert missing == set()
    assert q.as_string() == 'WHERE "substr"=%(substr)s AND "substr" LIKE %(substr_contains)s'
    assert subdict == { 'substr': 'B', 'substr_contains': '%C%' }

    q, subdict, missing = construct_pgsql_where_clause( searchspec, multsubstr="C", multsubstr_contains="D" )
    assert missing == set()
    ...
    q, subdict, missing = construct_pgsql_where_clause( searchspec, multsubstr="C", multsubstr_contains=[ "D", "E" ] )
    assert missing == set()
